...
extern int __pyx_module_is_main_mymath;
...
__declspec(dllexport) int add(int __pyx_v_a, int __pyx_v_b) {
  int __pyx_r;
...
__Pyx_RefNannySetupContext("PyInit_mymath", 0);
...
#endif
{
...
#endif
    if (PyImport_AppendInittab("mymath", PyInit_mymath) < 0) return 1;
#if PY_VERSION_HEX < 0x03080000
    Py_Initialize();
    if (argc && argv)
    ...
    {
      PyObject* m = NULL;
      __pyx_module_is_main_mymath = 1;
      m = PyImport_ImportModule("mymath");
      if (!m && PyErr_Occurred()) {
          PyErr_Print();
          return 1;
      }
