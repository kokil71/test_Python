...
def extract_text_from_image(converted_image, lang="eng"):
    ...
    plt_imshow(["Original", "Blackhat", "Gradient", "Rect Close", "Square Close"], [converted_image, blackhat, grad, thresh, close_thresh], figsize=(16, 10))
    ...
    Convert_grouping = converted_image.copy()
    ...
    for c in cnts:
        ...
        if ar > 3.0 and ar < 6.5 and (W/2) < x:
            ...
            roi = converted_image[y - margin:y + h + margin, x - margin:x + w + margin]
            roi_list.append(roi)
        ...
        cv2.rectangle(Convert_grouping, (x - margin, y - margin), (x + w + margin, y + h + margin), color, 2)
        cv2.putText(Convert_grouping, "".join(str(ar)), (x, y - 15), cv2.FONT_HERSHEY_SIMPLEX, 0.65, color, 2)

    #plt_imshow(["Grouping Image"], [receipt_grouping], figsize=(16, 10))
    plt_imshow(["Grouping Image"], [Convert_grouping], figsize=(16, 10))
    ...
    text = pytesseract.image_to_string(cv2.cvtColor(converted_image, cv2.COLOR_BGR2RGB), lang=lang, config=options)

    # OCR결과 출력
    #print("[INFO] OCR 결과:")
    print("[INFO] 이미지 처리 기술 적용 OCR 결과:")
    print("==================")
    print(text)
    print("\n")
...
def pint_text_extracted_from_image(text, option):
    match option:
        ...
        case 2:
            ...
            print("이 메 일 : {}".format(emails))
            print("주    소 : {}".format(addr))
        case _:
            print("")
...
# 이미지 처리 기술과 OpenCV 라이브러리를 사용하여 입력 이미지에서 원하는 텍스트를 추출 하는 방법
text = extract_text_from_image(receipt_image)
# 정규식을 통한 영역 추출
pint_text_extracted_from_image(text, 1)
...
print("\n")
# 이미지 처리 기술과 OpenCV 라이브러리를 사용하여 입력 이미지에서 원하는 텍스트를 추출 하는 방법
text = extract_text_from_image(receipt_image, lang="eng")
# 정규식을 통한 영역 추출
pint_text_extracted_from_image(text, 1)
...
# 수정된 이미지를 OCR로 수행하면 아래와 같은 결과가 나옵니다.
options = "--psm 4"
text = pytesseract.image_to_string(cv2.cvtColor(business_card_image, cv2.COLOR_BGR2RGB), lang="kor+eng", config=options)
# OCR결과 출력
print("[INFO] OCR결과:")
print("==================")
print(text)
print("\n")
# 이미지 처리 기술과 OpenCV 라이브러리를 사용하여 입력 이미지에서 원하는 텍스트를 추출 하는 방법
text = extract_text_from_image(business_card_image, lang="kor+eng")
# 정규식을 통한 영역 추출
pint_text_extracted_from_image(text, 2)

"""

# Image Load - 
imPath = r"d:\work\GeoData\ocr_test2.jpg"       # 로컬 영수증
#imPath = r"d:\work\GeoData\ocr_test1.jpg"       # 로컬 명  함
image_nparray = np.asarray(bytearray(open(imPath, "rb").read()), dtype=np.uint8)
org_image = cv2.imdecode(image_nparray, cv2.IMREAD_COLOR) 
plt_imshow("orignal image", org_image)
business_card_image2 = make_scan_image(org_image, width=200, ksize=(5, 5), min_threshold=20, max_threshold=100)
# 수정된 이미지를 OCR로 수행하면 아래와 같은 결과가 나옵니다.
options = "--psm 4"
text = pytesseract.image_to_string(cv2.cvtColor(business_card_image2, cv2.COLOR_BGR2RGB), lang="eng", config=options)       # 영수증
#text = pytesseract.image_to_string(cv2.cvtColor(business_card_image2, cv2.COLOR_BGR2RGB), lang="kor+eng", config=options)   # 명  함
# OCR결과 출력
print("[INFO] OCR결과:")
print("==================")
print(text)
print("\n")
# 이미지 처리 기술과 OpenCV 라이브러리를 사용하여 입력 이미지에서 원하는 텍스트를 추출 하는 방법
text = extract_text_from_image(business_card_image2, lang="eng")   # 영수증
#text = extract_text_from_image(business_card_image2, lang="kor+eng")   # 명  함
# 정규식을 통한 영역 추출
pint_text_extracted_from_image(text, 1)     # 영수증
#pint_text_extracted_from_image(text, 2)     # 명  함