...
class ExcelToAutoCADTable:
    def __init__(self, excel_path, sheet_name, cell_range):
        ...
        self.data, self.alignments, self.merged = self._read_excel()
        ...
        for r in merged_ranges:
            ...
            for row in range(minr, maxr + 1):
                for col in range(minc, maxc + 1):
                    merged_map[(row, col)] = (minr, minc)  # 대표 셀 위치
        ...
        for i, row in enumerate(ws.iter_rows(min_row=min_row, max_row=max_row,
            ...
            for j, cell in enumerate(row, start=min_col):
                if (i, j) in merged_map and (i, j) != merged_map[(i, j)]:
                    drow.append(None)  # 병합된 셀의 중복 위치는 건너뜀
                    arow.append(None)
                    continue
    ...
    def draw_in_cad(self, start_point=(0, 0), style_name="HangulTTF", font_file="malgun.ttf"):
        ...
        for row_idx, row in enumerate(self.data):
            ...
            for col_idx, text in enumerate(row):
                ...
                col_abs = col_idx + 1
                rowspan, colspan = 1, 1
                if (row_abs, col_abs) in self.merged:
                    base_r, base_c = self.merged[(row_abs, col_abs)]
                    if (row_abs, col_abs) != (base_r, base_c):
                        continue
                    # 병합 셀의 범위 구함
                    for rng in doc.ActiveLayout.Block.Document.Sheets(self.sheet_name).Cells.MergedCells:
                        if rng.TopLeftCell.Row == base_r and rng.TopLeftCell.Column == base_c:
                            rowspan = rng.Rows.Count
                            colspan = rng.Columns.Count
                            break
